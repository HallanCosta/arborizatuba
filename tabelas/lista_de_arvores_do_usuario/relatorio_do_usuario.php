...
$id_usuario = filter_input(INPUT_SESSION, 'id', FILTER_VALIDATE_INT);

if (!empty($page) && !empty($perPage)) {
	$start = $page > 1 ? ($page * $perPage) - $perPage : 0;
	$sql = "SELECT * FROM `validadas` WHERE 1 = 1 AND id_usuario = ?";

	$rows = DB::selectDB($sql, [$id_usuario], \PDO::FETCH_OBJ);
	$pages = ceil(COUNT($rows)/$perPage);
	if (COUNT($rows) > 0) {
		$sql = "SELECT * FROM `validadas` WHERE 1 = 1 AND id_usuario = ? LIMIT {$start}, {$perPage}";
		$rows = DB::selectDB($sql, [$id_usuario], \PDO::FETCH_OBJ);
	}
} elseif ($page == 0) {
...
if (filter_input(INPUT_POST, 'btnPesquisar')) {
	...
	$sql = "SELECT * FROM `validadas` WHERE 1 = 1 AND id_usuario = ?" . $txtPesquisa;
	$rows = DB::selectDB($sql, [$id_usuario], \PDO::FETCH_OBJ);
	if (COUNT($rows) > 0) {
	...
	switch ($msg) {
		case 1:
		echo 'entrei';
		$sql = "SELECT SUM(quant) AS totalArvoresTabela FROM `validadas` WHERE 1 = 1 AND id_usuario = ?";
		$resultTable = DB::selectDB($sql, [$id_usuario], \PDO::FETCH_OBJ);
		if (COUNT($resultTable) > 0) {
			$row = $resultTable[0];
			$totalArvoresTabela = $row->totalArvoresTabela;
		}

		$sql = "SELECT SUM(quant) AS totalArvoresPesquisa FROM `validadas` WHERE 1 = 1 AND id_usuario = ?" . $txtPesquisa ;
		$resultSearch = DB::selectDB($sql, [$id_usuario], \PDO::FETCH_OBJ);
				...
				<?php if (isset($totalArvoresTabela)): ?>
					<div class="alert alert-info">
						<span class="text-left">
							<h3>Total de Arvores da Tabela: <?=$totalArvoresTabela;unset($totalArvoresTabela);?></h3>
						</span>
					</div>
				<?php endif; ?>
						...
						<tbody>
							<?php foreach ($rows as $key => &$value): ?>
								<tr>
									...
									<td class="hidden-print"><a href="atualizar_dados.php?id_arvore=<?=$value->id;?>&id_usuario=<?=$value->id_usuario;?>" class="btn btn-warning pull-right">Editar</a></td>
								</tr>
							<?php endforeach; ?>
						</tbody>
					</table>
