...
//Identificação de Pagina
//$_VAR['idenfied'] = 'relatorio_validacao';
...
if (!empty($page) && !empty($perPage)) {
	...
	//AINDA PRRECISO ORDERNAR A SELEÇÃO LEVANDO EM CONTA A PODA MAIS URGENTE
	//DATA MAIS PROXIMA DA PROXIMA PODA
	...
	header('Location: relatorio_validacao?page=1');
} else {
	exit();
}
...
if (filter_input(INPUT_POST, 'btnPesquisar')) {
	...
	$sql = "SELECT * FROM `validacao` WHERE 1 = 1 " . $txtPesquisa;
	$rows = DB::selectDB($sql, NULL, \PDO::FETCH_OBJ);
	if (COUNT($rows) > 0) {
	...
	switch ($msg) {
		case 1:
		$sql = "SELECT SUM(quant) AS totalArvoresTabela FROM `validacao` WHERE 1 = 1";
		...
		$sql = "SELECT SUM(quant) AS totalArvoresPesquisa FROM `validacao` WHERE 1 = 1" . $txtPesquisa;
								...
								<?php foreach ($rows as $key => &$value): ?>
									<tr>
										...
										<?php if ($_SESSION['cargo'] == 2): ?>
											...
											<td class="hidden-print"><a href="descartar_arvore.php?id_arvore=<?= $value->id; ?>&id_usuario=<?= $value->id_usuario ?>" class="btn btn-danger">Descartar</a></td>
										<?php endif; ?>
									</tr>
								<?php endforeach; ?>
							</tbody>
