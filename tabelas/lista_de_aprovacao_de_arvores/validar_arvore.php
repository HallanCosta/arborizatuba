...
if (!empty($id_arvore) && !empty($id_usuario)) {
	...
	if (COUNT($rows) > 0) {
		...
		if ($lastInsertId > 0) {
			//print('Inserido na Tabela Validados');
			//print('Trigger Insere na Tabela Fotos_Validadas');
			//NÃO É NECESSARIO INSERIR EM CONF_PODA (O TRIGGER JÁ FAZ ISSO)
			//print('Inserido na Tabela conf_poda');
			...
			if ($affectedRow > 0) {
				...
				print('<div class="alert alert-danger">
					Houve um erro: Falha ao deletar os dados na Tabela `Validacao`.
					</div>
					');
			}
